...
int main(int argc, char *argv[]) try {
  ...
  auto norm2 = [N = opts.N](const auto &vec) -> auto {
    double sum = 0.0;
    #pragma omp parallel for collapse(2) reduction(+:sum) schedule(static,1)
    for (size_t j = 0; j < N; ++j)
      for (size_t i = 1; i < (N - 1); ++i)
  ...
  auto normInf = [N = opts.N](const auto &vec) -> auto {
    double max = 0.0;
    #pragma omp parallel for collapse(2) reduction(+:max) schedule(static,1)
    for (size_t j = 0; j < N; ++j)
      for (size_t i = 1; i < (N - 1); ++i)
  ...
  std::cout << "2-norm = " << norm2(x2) << std::endl;
  std::cout << "inf-norm = " << normInf(x2) << std::endl;
